...
use Temkaa\Botifier\Factory\Shared\ChatPermissionsFactory;
...
use Temkaa\Botifier\Model\Response\Nested\ChatFullInfo;
...
final readonly class ChatFullInfoFactory
{
    ...
    public function create(array $message): ChatFullInfo
    {
        $factory = match (true) {
            ...
            is_array($message['available_reactions']) && $message[0]['type'] === 'emoji'        => $this->reactionTypeEmojiFactory,
            is_array($message['available_reactions']) && $message[0]['type'] === 'custom_emoji' => $this->reactionTypeCustomEmojiFactory,
            is_array($message['available_reactions']) && $message[0]['type'] === 'paid'         => $this->reactionTypePaidFactory,
            default                                                                             => null,
        };
        ...
        return new ChatFullInfo(
            ...
            isset($message['business_location']) ? $this->businessLocationFactory->create($message['business_location']) : null,
            isset($message['business_opening_hours']) ? $this->businessOpeningHoursFactory->create($message['business_opening_hours']) : null,
            ...
            isset($message['emoji_status_expiration_date']) ? (new DateTimeImmutable())->setTimestamp($message['emoji_status_expiration_date'])->setTimezone(new DateTimeZone('UTC')) : null,
            $message['bio'] ?? null,
