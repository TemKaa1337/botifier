...
final readonly class WebhookInfoFactory
{
    public function create(array $message): WebhookInfo
    {
        return new WebhookInfo(
            ...
            isset($message['last_error_date']) ? (new DateTimeImmutable())->setTimestamp($message['last_error_date'])->setTimezone(new DateTimeZone('UTC')) : null,
            $message['last_error_message'] ?? null,
            isset($message['last_synchronization_error_date']) ? (new DateTimeImmutable())->setTimestamp($message['last_synchronization_error_date'])->setTimezone(new DateTimeZone('UTC')) : null,
