...
use Temkaa\Botifier\Model\Response\Nested\ExternalReplyInfo;
...
final readonly class ExternalReplyInfoFactory
{
    ...
    public function create(array $message): ExternalReplyInfo
    {
        return new ExternalReplyInfo(
            match (true) {
                $message['origin']['type'] === 'user'        => $this->messageOriginUserFactory->create($message['origin']),
                $message['origin']['type'] === 'hidden_user' => $this->messageOriginHiddenUserFactory->create($message['origin']),
                $message['origin']['type'] === 'chat'        => $this->messageOriginChatFactory->create($message['origin']),
                $message['origin']['type'] === 'channel'     => $this->messageOriginChannelFactory->create($message['origin']),
